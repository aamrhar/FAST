...
%% Georges Brown : Edited the option to fit ublox GPS -> GPGGA -> GNGGA
% Modified by abdu
nmea_options  =  [  '$GNGGA'
                    '$GNGLL'
                    '$GNGSA'
...
% added by abdu 
case_t2 = find(strcmp(fields{1}(1), nmea_options2),1);
if isempty(case_t)
    case_t=case_t2;
end
...
checksum_Ublox = nline(find(nline=='*',1,'last')+1:find(nline=='*',1,'last')+2);

%check it
if ((strcmp(checksum_Ublox, checksum))==0)
   %checksum is bad!
